...
namespace DriverTest
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            using (S7CommPlusSafeClient client = new S7CommPlusSafeClient())
            {
                res = client.Connect(HostIp, Password, Username);
                if (res == 0)
                {
                    ...
                    res = client.ReadValues(readlist, out values, out errors);
                    #endregion
                    ...
                    errors.Clear();
                    res = client.WriteValues(writelist, writevalues, out errors);
                    ...
                    */
                    ...
                    client.Connection.SetPlcOperatingState(1);
                    Console.WriteLine("Taste drücken um wieder in RUN zu setzen...");
                    Console.ReadKey();
                    Console.WriteLine("Setze SPS in RUN...");
                    client.Connection.SetPlcOperatingState(3);
                    #endregion
                    */
                }
                else
                {
                    Console.WriteLine("Main - Connect fehlgeschlagen! Error=" + res + ", Text=" + S7Client.ErrorText(res));
                }
            }
